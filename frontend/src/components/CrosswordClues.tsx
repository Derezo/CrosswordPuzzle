...
interface ClueRowProps {
  ...
  isCompleted: boolean;
  isFocused: boolean;
  validationState: boolean | undefined; // true=correct, false=incorrect, undefined=unchecked
  onClueClick: (clue: CrosswordClue) => void;
  onFeedbackClick?: (clue: CrosswordClue) => void;
  onRevealClick?: (clue: CrosswordClue) => void;
  revealDisabled?: boolean;
}

// Memoized so a single focus / completion change re-renders only the affected
// row instead of the whole clue list. Reference-stable callbacks from the
// parent (or stable scalar deps) are required for the memo to bite.
const ClueRow = React.memo(function ClueRow({
  clue,
  isCompleted,
  isFocused,
  validationState,
  ...
  let validationClass = '';
  if (isCompleted || validationState === true) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 relative">
        ...
        {!isCompleted && onRevealClick && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (!revealDisabled) onRevealClick(clue);
            }}
            disabled={revealDisabled}
            className={clsx(
              'text-xs p-1 rounded transition-colors flex-shrink-0',
              revealDisabled
                ? 'text-yellow-700 opacity-50 cursor-not-allowed'
                : 'text-yellow-300 hover:text-yellow-200 hover:bg-yellow-500/20',
            )}
            title={
              revealDisabled
                ? 'Reveal limit reached or cooling down'
                : 'Reveal one letter (disqualifies this puzzle from achievements)'
            }
          >
            💡
          </button>
        )}
        {!isCompleted && validationState === false && (
...
interface CrosswordCluesProps {
  ...
  onFeedbackClick?: (clue: CrosswordClue) => void;
  onRevealClick?: (clue: CrosswordClue) => void;
  // Truthy when the user has hit cooldown / daily cap.
  revealDisabled?: boolean;
}
...
export const CrosswordClues: React.FC<CrosswordCluesProps> = ({
  ...
  // Each clue's "was this validated correct / incorrect?" status comes from
  // progress.validatedClues so that:
  // 1. The red X / green ✓ pill survives a page reload.
  // 2. Each clue reads only its own status — a wrong crossing word can no
  //    longer poison the indicator on a correctly completed word.
  const validatedClues = progress.validatedClues ?? {};
  ...
  const renderClueRow = (clue: CrosswordClue, keyPrefix = '') => (
    <ClueRow
      key={`${keyPrefix}${clue.direction}-${clue.number}`}
      clue={clue}
      isCompleted={completedSet.has(clue.number)}
      isFocused={
        focusedClue?.number === clue.number &&
        focusedClue?.direction === clue.direction
      }
      validationState={validatedClues[clue.number]}
  ...
  const ClueSection: React.FC<{ title: string; clues: CrosswordClue[] }> = ({ title, clues: list }) => (
    <div className="mb-3">
      ...
      <div className="space-y-1">
        {list.map((clue) => renderClueRow(clue))}
      </div>
    </div>
  );
  ...
  return (
    ...
    <div className="relative max-h-[60vh] lg:max-h-full overflow-y-auto custom-scrollbar overscroll-contain"
      ...
      <div className="lg:hidden space-y-1">
        {mobileList.map((clue) => renderClueRow(clue, 'm-'))}
      </div>
