...
class StrictCrosswordGenerator {
    ...
    placeWord(entry, row, col, direction, intersections) {
        ...
        if (direction === "across") {
            ...
            if (col + entry.word.length < GRID_SIZE &&
            ...
            if (row + entry.word.length < GRID_SIZE &&
    ...
    buildPuzzle() {
        ...
        const seedWords = this.wordsByLength.get(7) ||
        ...
        while (this.placedWords.length < targetWords &&
            ...
            for (let c = 0; c <= GRID_SIZE; c++) {
                ...
                if (!isEnd) {
                    ...
                    if (!processedWords.has(key)) {
                        ...
                        const placedWord = this.placedWords.find((w) => w.row === r &&
                            w.col === startCol &&
                            w.direction === "across" &&
                            w.word === word);
                        if (placedWord) {
            ...
            for (let r = 0; r <= GRID_SIZE; r++) {
                ...
                if (!isEnd) {
                    ...
                    if (!processedWords.has(key)) {
                        processedWords.add(key);
                        // Find matching placed word
                        const placedWord = this.placedWords.find((w) => w.row === startRow &&
                            w.col === c &&
                            w.direction === "down" &&
                            w.word === word);
                        if (placedWord) {
