...
class ImprovedCrosswordGenerator {
    ...
    tryPlaceCrossingWord(letter, crossRow, crossCol, direction) {
        ...
        for (const { entry, position } of candidates.slice(0, 50)) { // Try top 50
            ...
            if (direction === 'across') {
                if (startCol > 0 && !this.grid[startRow][startCol - 1].isBlocked && this.grid[startRow][startCol - 1].letter)
                    continue;
                if (startCol + word.length < GRID_SIZE && !this.grid[startRow][startCol + word.length].isBlocked && this.grid[startRow][startCol + word.length].letter)
                    continue;
            }
            else {
                if (startRow > 0 && !this.grid[startRow - 1][startCol].isBlocked && this.grid[startRow - 1][startCol].letter)
                    continue;
                if (startRow + word.length < GRID_SIZE && !this.grid[startRow + word.length][startCol].isBlocked && this.grid[startRow + word.length][startCol].letter)
                    continue;
            }
            // Place the word
            for (let i = 0; i < word.length; i++) {
