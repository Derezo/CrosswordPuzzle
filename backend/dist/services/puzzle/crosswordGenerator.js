...
class CrosswordGenerator {
    ...
    findWordSlots() {
        ...
        while (i < acrossSlots.length || j < downSlots.length) {
            if (i < acrossSlots.length)
                interleavedSlots.push(acrossSlots[i++]);
            if (j < downSlots.length)
                interleavedSlots.push(downSlots[j++]);
        }
...
function generateDailyPuzzle(date) {
    const generator = new improvedCrosswordGenerator_1.ImprovedCrosswordGenerator(date);
