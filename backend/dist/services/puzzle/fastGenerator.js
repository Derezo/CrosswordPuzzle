...
exports.FastCrosswordGenerator = void 0;
exports.generateFastDailyPuzzle = generateFastDailyPuzzle;
...
class FastCrosswordGenerator {
        ...
        while (iterations < maxIterations && placedWords.length < 60) {
            ...
            for (const targetLength of [5, 6, 4, 7, 8, 3, 9, 10, 11, 12]) {
                ...
                for (const slot of slots.slice(0, 10)) { // Try more slots per length
                    ...
                    const candidates = availableWords.filter(word => !placedWords.some(p => p.word === word) && // Not already used
                ...
                if (placedThisRound)
                    break; // Try again with all lengths
            }
            if (!placedThisRound) {
    ...
    countIntersections(row, col, length, direction) {
        let count = 0;
            ...
            for (let col = 0; col <= GRID_SIZE; col++) {
                if (col === GRID_SIZE || this.grid[row][col].isBlocked) {
                    if (start !== -1 && col - start === length) {
                        slots.push({ row, col: start, direction: 'across', length });
                    }
                    start = -1;
                }
                else if (start === -1) {
            ...
            for (let row = 0; row <= GRID_SIZE; row++) {
                if (row === GRID_SIZE || this.grid[row][col].isBlocked) {
                    if (start !== -1 && row - start === length) {
                        slots.push({ row: start, col, direction: 'down', length });
                    }
                    start = -1;
                }
                else if (start === -1) {
    ...
    isSlotOccupied(row, col, length, direction) {
        for (let i = 0; i < length; i++) {
            const r = direction === 'across' ? row : row + i;
            const c = direction === 'across' ? col + i : col;
            if (this.grid[r][c].letter)
    ...
    getCrossWord(row, col, direction) {
        ...
        while (start > 0) {
            ...
            if (this.grid[r][c].isBlocked || (!this.grid[r][c].letter && start === (direction === 'across' ? col : row)))
                break;
            start--;
        }
        // Find end
        while (end < GRID_SIZE - 1) {
            ...
            if (this.grid[r][c].isBlocked || (!this.grid[r][c].letter && end === (direction === 'across' ? col : row)))
                break;
            end++;
        }
        if (end - start + 1 < MIN_WORD_LENGTH)
            return null;
        let word = '';
...
exports.FastCrosswordGenerator = FastCrosswordGenerator;
function generateFastDailyPuzzle(date) {
...
//# sourceMappingURL=fastGenerator.js.map