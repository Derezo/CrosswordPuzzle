...
export class ImprovedCrosswordGenerator {
  ...
  private loadDictionary(): void {
    ...
    console.log(
      `📚 Loaded ${this.dictionary.length} words (3-${MAX_WORD_LENGTH} letters)`,
    );
  }
  ...
  private placeSeedPattern(): void {
    ...
    const seedWords =
      this.wordsByLength.get(7) || this.wordsByLength.get(6) || [];
    ...
    if (startCol + seedEntry.word.length < GRID_SIZE) {
      this.grid[symRow][
        GRID_SIZE - 1 - (startCol + seedEntry.word.length)
      ].isBlocked = true;
    }
  }
   ...
   */
  private findCrossingOpportunities(): Array<{
    letter: string;
    row: number;
    col: number;
    direction: "across" | "down";
    ...
    for (const { entry, position } of candidates.slice(0, 50)) {
      ...
      if (direction === "across") {
        if (
          ...
          continue;
        if (
          startCol + word.length < GRID_SIZE &&
        ...
        if (
          ...
          continue;
        if (
          startRow + word.length < GRID_SIZE &&
      ...
      if (direction === "across") {
        ...
        if (
          ...
          // Add symmetric
          this.grid[GRID_SIZE - 1 - startRow][
            GRID_SIZE - 1 - (startCol + word.length)
          ].isBlocked = true;
        }
      } else {
        ...
        if (
          ...
          // Add symmetric
          this.grid[GRID_SIZE - 1 - (startRow + word.length)][
            GRID_SIZE - 1 - startCol
          ].isBlocked = true;
        }
      }
  ...
  private buildPuzzle(): void {
    ...
    while (iterations < maxIterations && this.placedWords.length < 30) {
      ...
      for (const opp of opportunities.slice(0, 10)) {
        // Try top 10
        if (
          this.tryPlaceCrossingWord(opp.letter, opp.row, opp.col, opp.direction)
        ) {
          placed = true;
          break;
        }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (score > bestScore) {
        ...
        if (acrossCount >= 10 && downCount >= 10) {
          console.log(
            `✅ Generated balanced puzzle: ${acrossCount} across, ${downCount} down (attempt ${attempt + 1})`,
          );
          return bestPuzzle;
        }
      }
