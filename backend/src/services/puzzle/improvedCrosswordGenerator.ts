...
export class ImprovedCrosswordGenerator {
   ...
   */
  private findCrossingOpportunities(): Array<{letter: string, row: number, col: number, direction: 'across' | 'down'}> {
    const opportunities: Array<{letter: string, row: number, col: number, direction: 'across' | 'down'}> = [];
    ...
    for (const word of this.placedWords) {
      for (let i = 0; i < word.word.length; i++) {
        ...
        if (word.direction === 'across') {
          ...
          // Check above
          if (word.row > 0 && !this.grid[word.row - 1][col].isBlocked && !this.grid[word.row - 1][col].letter) {
            opportunities.push({ letter, row: word.row, col, direction: 'down' });
          }
          // Check below
          if (word.row < GRID_SIZE - 1 && !this.grid[word.row + 1][col].isBlocked && !this.grid[word.row + 1][col].letter) {
            opportunities.push({ letter, row: word.row, col, direction: 'down' });
          }
        } else {
          ...
          // Check left
          if (word.col > 0 && !this.grid[row][word.col - 1].isBlocked && !this.grid[row][word.col - 1].letter) {
            opportunities.push({ letter, row, col: word.col, direction: 'across' });
          }
          // Check right  
          if (word.col < GRID_SIZE - 1 && !this.grid[row][word.col + 1].isBlocked && !this.grid[row][word.col + 1].letter) {
            opportunities.push({ letter, row, col: word.col, direction: 'across' });
          }
        }
      }
   ...
   */
  private tryPlaceCrossingWord(letter: string, crossRow: number, crossCol: number, direction: 'across' | 'down'): boolean {
    // Find words containing this letter
    const candidates: Array<{entry: DictionaryEntry, position: number}> = [];
    
    for (const [length, entries] of this.wordsByLength) {
      if (length > MAX_WORD_LENGTH) continue;
      
      for (const entry of entries) {
        // Skip already used words
        if (this.placedWords.some(p => p.word === entry.word)) continue;
    ...
    // Try each candidate
    for (const { entry, position } of candidates.slice(0, 50)) { // Try top 50
      ...
      if (direction === 'across' && startCol + word.length > GRID_SIZE) continue;
      if (direction === 'down' && startRow + word.length > GRID_SIZE) continue;
      ...
      if (direction === 'across') {
        if (startCol > 0 && !this.grid[startRow][startCol - 1].isBlocked && this.grid[startRow][startCol - 1].letter) continue;
        if (startCol + word.length < GRID_SIZE && !this.grid[startRow][startCol + word.length].isBlocked && this.grid[startRow][startCol + word.length].letter) continue;
      } else {
        if (startRow > 0 && !this.grid[startRow - 1][startCol].isBlocked && this.grid[startRow - 1][startCol].letter) continue;
        if (startRow + word.length < GRID_SIZE && !this.grid[startRow + word.length][startCol].isBlocked && this.grid[startRow + word.length][startCol].letter) continue;
      }
      ...
      if (direction === 'across') {
        if (startCol > 0 && !this.grid[startRow][startCol - 1].letter) {
          ...
          // Add symmetric
          this.grid[GRID_SIZE - 1 - startRow][GRID_SIZE - startCol].isBlocked = true;
        }
        if (startCol + word.length < GRID_SIZE && !this.grid[startRow][startCol + word.length].letter) {
        ...
        if (startRow > 0 && !this.grid[startRow - 1][startCol].letter) {
          ...
          // Add symmetric
          this.grid[GRID_SIZE - startRow][GRID_SIZE - 1 - startCol].isBlocked = true;
        }
        if (startRow + word.length < GRID_SIZE && !this.grid[startRow + word.length][startCol].letter) {
  ...
  private buildPuzzle(): void {
    ...
    while (iterations < maxIterations && this.placedWords.length < 30) {
      ...
      let placed = false;
      for (const opp of opportunities.slice(0, 10)) { // Try top 10
...
// Export function
export function generateImprovedPuzzle(date: string): GeneratedPuzzle {
