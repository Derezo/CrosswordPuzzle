...
export class StrictCrosswordGenerator {
  ...
  private canPlaceWord(word: string, row: number, col: number, direction: 'across' | 'down'): {valid: boolean, intersections: Array<{row: number, col: number}>} {
    // Check bounds
    if (row < 0 || col < 0) return {valid: false, intersections: []};
    if (direction === 'across' && col + word.length > GRID_SIZE) return {valid: false, intersections: []};
    if (direction === 'down' && row + word.length > GRID_SIZE) return {valid: false, intersections: []};
    ...
    if (direction === 'across') {
      // Check before
      if (col > 0 && !this.grid[row][col-1].isBlocked && this.grid[row][col-1].letter) {
        return {valid: false, intersections: []};
      }
      // Check after
      if (col + word.length < GRID_SIZE && !this.grid[row][col + word.length].isBlocked && this.grid[row][col + word.length].letter) {
        return {valid: false, intersections: []};
      }
    } else {
      // Check before
      if (row > 0 && !this.grid[row-1][col].isBlocked && this.grid[row-1][col].letter) {
        return {valid: false, intersections: []};
      }
      // Check after
      if (row + word.length < GRID_SIZE && !this.grid[row + word.length][col].isBlocked && this.grid[row + word.length][col].letter) {
        return {valid: false, intersections: []};
      }
    }
  ...
  private placeWord(entry: DictionaryEntry, row: number, col: number, direction: 'across' | 'down', intersections: Array<{row: number, col: number}>): void {
    // Place letters
    ...
    if (direction === 'across') {
      ...
      if (col + entry.word.length < GRID_SIZE && !this.grid[row][col + entry.word.length].letter) {
      ...
      if (row + entry.word.length < GRID_SIZE && !this.grid[row + entry.word.length][col].letter) {
  ...
  private findAllPlacements(entry: DictionaryEntry): Array<{row: number, col: number, direction: 'across' | 'down', intersections: Array<{row: number, col: number}>, score: number}> {
    const placements: Array<{row: number, col: number, direction: 'across' | 'down', intersections: Array<{row: number, col: number}>, score: number}> = [];
  ...
  private wouldCreateInvalidWord(entry: DictionaryEntry, row: number, col: number, direction: 'across' | 'down'): boolean {
    ...
    if (direction === 'across') {
      ...
      for (let i = 0; i < entry.word.length; i++) {
        ...
        // Find start
        while (startR > 0 && !tempGrid[startR-1][c].isBlocked && tempGrid[startR-1][c].letter) {
          startR--;
        }
        
        // Find end
        while (endR < GRID_SIZE - 1 && !tempGrid[endR+1][c].isBlocked && tempGrid[endR+1][c].letter) {
          endR++;
        }
      ...
      for (let i = 0; i < entry.word.length; i++) {
        ...
        // Find start
        while (startC > 0 && !tempGrid[r][startC-1].isBlocked && tempGrid[r][startC-1].letter) {
          startC--;
        }
        
        // Find end
        while (endC < GRID_SIZE - 1 && !tempGrid[r][endC+1].isBlocked && tempGrid[r][endC+1].letter) {
          endC++;
        }
  ...
  private buildPuzzle(): boolean {
    ...
    const seedWords = this.wordsByLength.get(7) || this.wordsByLength.get(6) || this.wordsByLength.get(8) || [];
    if (seedWords.length === 0) return false;
    
    // Place first word
    const firstWord = seedWords[Math.floor(this.rng() * Math.min(50, seedWords.length))];
    ...
    while (this.placedWords.length < targetWords && consecutiveFailures < maxConsecutiveFailures) {
      const candidates = this.findBestCandidates();
      let placed = false;
      
      // Try each candidate
      for (const entry of candidates.slice(0, 200)) { // Try top 200 candidates
        ...
        for (const placement of placements.slice(0, 5)) {
          ...
          if (!this.wouldCreateInvalidWord(entry, placement.row, placement.col, placement.direction)) {
            this.placeWord(entry, placement.row, placement.col, placement.direction, placement.intersections);
            placed = true;
            break;
          }
        }
      ...
      for (let c = 0; c <= GRID_SIZE; c++) {
        ...
        if (!isEnd) {
          ...
          if (!processedWords.has(key)) {
            ...
            const placedWord = this.placedWords.find(w => 
              w.row === r && w.col === startCol && w.direction === 'across' && w.word === word
            );
      ...
      for (let r = 0; r <= GRID_SIZE; r++) {
        ...
        if (!isEnd) {
          ...
          if (!processedWords.has(key)) {
            ...
            // Find matching placed word
            const placedWord = this.placedWords.find(w => 
              w.row === startRow && w.col === c && w.direction === 'down' && w.word === word
            );
      ...
      if (this.buildPuzzle()) {
        ...
        if (this.validatePuzzle()) {
          ...
          if (clues.length >= 20) {
            ...
            console.log(`✅ Valid puzzle generated (attempt ${attempt + 1}): ${acrossCount} across, ${downCount} down`);
