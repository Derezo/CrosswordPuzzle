...
let WORDS_BY_FIRST_LETTER: Map<string, string[]> = new Map();
let WORDS_BY_LAST_LETTER: Map<string, string[]> = new Map();
...
try {
  ...
  if (fs.existsSync(dictPath)) {
    ...
    for (const word of DICTIONARY_WORDS) {
      ...
      if (!WORDS_BY_FIRST_LETTER.has(firstLetter)) WORDS_BY_FIRST_LETTER.set(firstLetter, []);
      if (!WORDS_BY_LAST_LETTER.has(lastLetter)) WORDS_BY_LAST_LETTER.set(lastLetter, []);
...
interface Intersection {
  slotId: string;
  myIndex: number;    // Position in this word
  theirIndex: number; // Position in the intersecting word
}
...
export class StandardCrosswordGenerator {
  ...
  private selectThemeWords(): ThemeWord[] {
    const themes = [
  ...
  private createSymmetricPattern(): void {
  ...
  private shouldPlaceBlackSquare(row: number, col: number): boolean {
  ...
  private findWordSlots(): WordSlot[] {
  ...
  private computeIntersections(slots: WordSlot[]): void {
  ...
  private findIntersection(slot1: WordSlot, slot2: WordSlot): { index1: number; index2: number } | null {
  ...
  private validateCrosswordStructure(slots: WordSlot[]): boolean {
    // Create a grid to track coverage
    const acrossCoverage: boolean[][] = [];
    const downCoverage: boolean[][] = [];
  ...
  private selectWordsForSlots(): boolean {
  ...
  private fillSlots(slots: WordSlot[], index: number): boolean {
  ...
  private getCandidateWords(slot: WordSlot): string[] {
  ...
  private getConstraints(slot: WordSlot): (string | null)[] {
    const constraints: (string | null)[] = new Array(slot.length).fill(null);
  ...
  private canAssignWord(slot: WordSlot, word: string): boolean {
    const constraints = this.getConstraints(slot);
  ...
  private unassignWord(slot: WordSlot): void {
  ...
  private addClueNumbers(): void {
    const numberedPositions = new Set<string>();
    const cluePositions: { row: number; col: number; number: number }[] = [];
  ...
  private createFallbackPuzzle(): GeneratedPuzzle {
    this.initializeGrid();
...
export function generateStandardDailyPuzzle(date: string): GeneratedPuzzle {
