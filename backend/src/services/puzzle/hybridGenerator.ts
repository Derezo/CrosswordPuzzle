...
interface WordSlot {
  ...
  direction: 'across' | 'down';
  length: number;
  constraints: (string | null)[]; // null = any letter, string = required letter
  intersections: Array<{
    slotId: string;
    myIndex: number;
    theirIndex: number;
...
class AdvancedCrosswordGenerator {
  ...
  private assignments: Map<string, string>; // slotId -> word
  private rng: () => number;
  private themeWord: { word: string; clue: string };
  ...
  private findValidWords(slot: WordSlot): string[] {
  ...
  private selectNextSlot(): WordSlot | null {
  ...
  private solveCSP(): boolean {
    const slot = this.selectNextSlot();
    if (!slot) return true; // All slots assigned
  ...
  private placeThemeWordFirst(): boolean {
    ...
    const suitableSlots = this.slots.filter(slot => 
      slot.length === this.themeWord.word.length
    );
    
    if (suitableSlots.length === 0) return false;
...
export function generateHybridDailyPuzzle(date: string): GeneratedPuzzle {
