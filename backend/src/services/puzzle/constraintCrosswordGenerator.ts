...
interface WordSlot {
  ...
  direction: 'across' | 'down';
  length: number;
  pattern: string; // Current pattern with '.' for unknowns
  intersections: Array<{
...
export class ConstraintCrosswordGenerator {
  ...
  private createSymmetricBlackPattern(): void {
  ...
  private getMatchingWords(slot: WordSlot): DictionaryEntry[] {
  ...
  private updatePattern(slot: WordSlot, word: string): void {
  ...
  private revertPattern(slot: WordSlot): void {
  ...
  private fillSlots(): boolean {
    // Use backtracking to fill all slots
    return this.backtrackFill(0);
  }
  
  private backtrackFill(slotIndex: number): boolean {
...
export function generateConstraintPuzzle(date: string): GeneratedPuzzle {
