...
interface ValidationResult {
  ...
  clueResults: { [clueNumber: number]: boolean };
  solvedClues: { [clueNumber: string]: string };
  newCompletedClues: number[];
}

export function validateGrid(
  userGrid: GridCellData[][],
  solutionGrid: any[][],
  cluesData: ClueData[],
  currentCompletedClues: number[]
): ValidationResult {
  
  // PURE GRID-BASED VALIDATION
  // Step 1: Compare user grid with solution grid cell by cell
  const cellValidation: { [key: string]: boolean } = {}; // "row,col": boolean
  const clueResults: { [key: number]: boolean } = {}; // clue validation results
  const solvedClues: { [clueNumber: string]: string } = {}; // extracted answers for UI
...
export function createSolutionGrid(
  ...
  const resultGrid: GridCellData[][] = solutionGrid.map(row => 
    row.map(cell => ({
